...
interface ContentContextType {
  ...
  interests: Interest[];
  addInterest: (interest: Omit<Interest, 'id' | 'date' | 'status'>) => void;
...
export function ContentProvider({ children }: { children: ReactNode }) {
  ...
  const [partners, setPartners] = useState<Partner[]>([
    { id: '1', name: 'UNICEF', type: 'international' },
    { id: '2', name: 'WHO', type: 'international' },
    { id: '3', name: 'UNHCR', type: 'international' },
    { id: '4', name: 'World Vision', type: 'international' },
    { id: '5', name: 'Save the Children', type: 'international' },
    { id: '6', name: 'Oxfam', type: 'international' },
    { id: '7', name: 'USAID', type: 'international' },
    { id: '8', name: 'Ministry of Health - DRC', type: 'government' },
    { id: '9', name: 'Ministry of Social Affairs - DRC', type: 'government' },
    { id: '10', name: 'North Kivu Provincial Government', type: 'government' },
    { id: '11', name: 'Goma Community Health Network', type: 'local' },
    { id: '12', name: 'Bukavu Women\'s Association', type: 'local' },
  ]);
  ...
  const [projects, setProjects] = useState<Project[]>([
    ...
    {
      id: '5',
      title: 'Refugee Camp Support Services',
    ...
    },
    {
      ...
      description: 'Empowering young people through leadership training, peace education, and civic engagement activities. This program builds the capacity of youth to become agents of positive change in their communities.',
  ...
  const updatePartner = (id: string, partner: Partial<Partner>) => {
    setPartners(partners.map(p => p.id === id ? { ...p, ...partner } : p));
  };
  ...
  const addNews = (newsItem: Omit<NewsItem, 'id'>) => {
    const newNewsItem = { ...newsItem, id: Date.now().toString() };
    setNews([...news, newNewsItem]);
  };

  const updateNews = (id: string, newsItem: Partial<NewsItem>) => {
    setNews(news.map(n => n.id === id ? { ...n, ...newsItem } : n));
  };
  ...
  const addImpactStory = (story: Omit<ImpactStory, 'id'>) => {
    ...
    setImpactStories([...impactStories, newStory]);
  };

  const updateImpactStory = (id: string, story: Partial<ImpactStory>) => {
    setImpactStories(impactStories.map(s => s.id === id ? { ...s, ...story } : s));
  };

  const deleteImpactStory = (id: string) => {
    setImpactStories(impactStories.filter(s => s.id !== id));
  };
  ...
  const addEnquiry = (enquiry: Omit<Enquiry, 'id' | 'date' | 'status'>) => {
    const newEnquiry = { ...enquiry, id: Date.now().toString(), date: new Date().toLocaleDateString(), status: 'new' };
  ...
  // Interests CRUD
  const addInterest = (interest: Omit<Interest, 'id' | 'date' | 'status'>) => {
    const newInterest = { ...interest, id: Date.now().toString(), date: new Date().toLocaleDateString(), status: 'new' };
    setInterests([...interests, newInterest]);
  };

  const updateInterestStatus = (id: string, status: Interest['status']) => {
    setInterests(interests.map(i => i.id === id ? { ...i, status } : i));
  };
